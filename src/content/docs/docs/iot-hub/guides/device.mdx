---
title: Device Library Contribution Guide
description: Build a ThingsBoard IoT Hub device package that end users install in one click.
---

Welcome to the **ThingsBoard Device Library**. This guide walks you through building a device package that end users can install on their ThingsBoard instance in a single click.

A device package is a ZIP file that bundles everything the installation wizard needs: configuration forms, setup instructions, ThingsBoard entity templates (device profiles, dashboards, rule chains), and images. When a user selects your device in the library, the wizard runs through your package step-by-step to provision the device, create a dashboard, and show the user how to connect the hardware.

## Quick Start

1. Pick an install method (e.g. `DIRECT_MQTT` for an MQTT-connected device).
2. Create a `device-info.json` that lists your device's metadata and install steps.
3. Write the supporting files: a prerequisites markdown, a configuration form, a device profile JSON, a device JSON, a dashboard JSON, and a post-install markdown.
4. Add a device photo under `images/` and reference it from `device-info.json`.
5. Zip the folder contents (not the folder itself) and upload via **IoT Hub → Add Item → Device**.

If your device has **one** way of connecting, put everything at the ZIP root. If it has **multiple** connectivity options (e.g. ChirpStack, TTN, and LORIOT), organize files into per-method subfolders plus a shared `common/` folder — see [Package Structure](#package-structure).

## How Users Install Your Device

When a user clicks **Install** on your device in the IoT Hub marketplace, the installation wizard performs these actions in order:
